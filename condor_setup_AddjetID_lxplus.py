"""
# How to run:
python3 condor_setup_AddjetID_lxplus.py
"""
import argparse
import os
import sys
...
def main(args):
    ...
    if EOS_Output_path == "":
        ...
        EOS_Output_path = '/eos/user/'+user_initials+'/'+username+'/AddedJetID_samples'
    if submission_name != "":
    ...
    post_proc_to_run = "add_jetid_v15.py"
    command = "python3 "+post_proc_to_run

    Transfer_Input_Files = post_proc_to_run   # FIXME: Generalise this.
    ...
    with open('input_data_Files/'+InputFileFromWhereReadDASNames) as in_file:
        ...
        outjdl_file.write("Transfer_Input_Files = " + Transfer_Input_Files + "\n")
    ...
    outScript.write("\n"+'echo "========================================="');
    outScript.write("\n"+'echo "cat '+post_proc_to_run+'"');
    outScript.write("\n"+'echo "..."');
    outScript.write("\n"+'cat '+post_proc_to_run);
    outScript.write("\n"+'echo "..."');
    outScript.write("\n"+'echo "========================================="');
    outScript.write("\n"+command + " -i ${1} -o . -m -1");
    ...
    outScript.write("\n"+'OUTFILE=$(ls *_jetId.root | head -n 1)');
    outScript.write("\n"+'if [ -z "$OUTFILE" ]; then');
    outScript.write("\n"+'    echo "ERROR: no *_jetId.root file produced"');
    outScript.write("\n"+'    echo "Current directory content:"');
    outScript.write("\n"+'    ls -alh');
    outScript.write("\n"+'    exit 1');
    outScript.write("\n"+'fi');
    outScript.write("\n"+'echo "Produced file: ${OUTFILE}"');
    outScript.write("\n"+'echo "Copy ${OUTFILE} to ${2}/${4}_jetID.root"');
    outScript.write("\n"+'cp "${OUTFILE}" "${2}/${4}_jetID.root"');
    outScript.write("\n"+'rm -f *.root');
...
if __name__ == "__main__":
    ...
    parser.add_argument("--submission_name", default="addedJetIDv15", help="String to be changed by user.")
    ...
    parser.add_argument("--eos_output_path", default='', help="EOS path for output files. By default it is `/eos/user/<UserInitials>/<UserName>/AddedJetID_samples`")
